...
# source: https://github.com/microsoft/onnxruntime/blob/main/tools/python/util/onnx_model_utils.py
...
def make_input_shape_fixed(model: onnx.ModelProto, fixed_shape: [int]):
    """
    ...
    # Remove output shape dim (assume this graph is valid)
    for dim in graph.output[0].type.tensor_type.shape.dim:
        if dim.HasField("dim_value") or dim.HasField("dim_param"):
            dim.Clear()

    # Force input shape dim (assume this graph is valid)
    for idx, dim in enumerate(graph.input[0].type.tensor_type.shape.dim):
...
def fix_output_shapes(model: onnx.ModelProto):
    """
    Update the output shapes of a model where the input shape/s were made fixed, if possible.
    ...
    try:
        m2 = onnx.shape_inference.infer_shapes(model, strict_mode=True, data_prop=True)
    except:
        print("failed")
        raise ValueError("erroneous model")
    # print("check model")
    # onnx.checker.check_model(m2, full_check=True)

    for idx, out in enumerate(model.graph.output):
        if not is_fixed_size_tensor(out):
            new_out = m2.graph.output[idx]
            if is_fixed_size_tensor(new_out):
                out.type.tensor_type.shape.CopyFrom(new_out.type.tensor_type.shape)
