...
def index_reverse(index: Tensor) -> Tensor:
...
def feature_shuffle(x, index: Tensor) -> Tensor:
    dim = index.dim()
    if x.shape[:dim] != index.shape:
        raise ValueError(
            "x ({:}) and index ({:}) shape incompatible"
            .format(x.shape, index.shape)
        )
...
def window_partition(x: Tensor, window_size) -> Tensor:
    """
    Args:
        x: (b, h, w, c)
...
class WindowAttention(nn.Module):
    ...
    def forward(self, qkv: Tensor, rpi, mask: Tensor | None = None) -> Tensor:
        r"""
        Args:
        ...
        relative_position_bias = (
            self.relative_position_bias_table[rpi.view(-1)]
            .view(
...
class ATD_CA(nn.Module):
    ...
    def forward(self, x: Tensor, td: Tensor, x_size: tuple[int, int]) -> Tensor:
        r"""
        Args:
...
class AC_MSA(nn.Module):
    ...
    def forward(self, qkv: Tensor, sim: Tensor, x_size: tuple[int, int]) -> Tensor:
        """
        Args:
        ...
        qkv = (
            y
            .reshape(b, ng, gs, 3, self.num_heads, c//self.num_heads)
            .permute(3, 0, 1, 4, 2, 5)  # 3, b, ng, nh, gs, c//nh
        )
        q, k, v = qkv[0], qkv[1], qkv[2]

        # Q @ K^T
        attn: Tensor = (q @ k.transpose(-2, -1))  # b, ng, nh, gs, gs
...
class ATDTransformerLayer(nn.Module):
    ...
    def forward(
        ...
        attn_windows: Tensor = (
            self.attn_win(x_windows, rpi=params['rpi_sa'], mask=attn_mask)
        )
...
class BasicBlock(nn.Module):
    ...
    def forward(self, x: Tensor, x_size, params) -> Tensor:
        b = x.shape[0]
...
class ATD(nn.Module):
    ...
    def forward_features(self, x: Tensor, params) -> Tensor:
    ...
    def calculate_mask(self, x_size: tuple[int, int]) -> Tensor:
        ...
        mask_windows: Tensor = window_partition(img_mask, self.window_size)
        mask_windows = mask_windows.view(-1, self.window_size * self.window_size)
        attn_mask: Tensor = mask_windows.unsqueeze(1) - mask_windows.unsqueeze(2)
        attn_mask = (
            attn_mask
            .masked_fill(attn_mask != 0, float(-100.0))
