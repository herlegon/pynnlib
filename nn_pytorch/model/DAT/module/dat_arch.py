...
def img2windows(img: Tensor, H_sp, W_sp):
    """
    Input: Image (B, C, H, W)
...
def windows2img(img_splits_hw: Tensor, H_sp, W_sp, H, W):
    """
...
class Spatial_Attention(nn.Module):
    ...
    def __init__(
        ...
        if idx == 0:
            ...
            raise RuntimeError(f"ERROR MODE:{idx}")
        self.H_sp = H_sp
        self.W_sp = W_sp
    ...
    def forward(self, qkv, H, W, mask=None):
        ...
        B, L, C = q.shape
        if L != H * W:
            raise RuntimeError("flatten img_tokens has wrong size")
...
class Adaptive_Spatial_Attention(nn.Module):
    ...
    def __init__(
        ...
        num_heads: int,
        reso: int = 64,
        split_size: tuple[int] = [8,8],
        shift_size: tuple[int] = [1,2],
        qkv_bias: bool = False,
        qk_scale: float | None = None,
        drop: float = 0.,
        attn_drop: float = 0.,
        ...
        if not (0 <= self.shift_size[0] < self.split_size[0]):
            raise ValueError("DAT: shift_size must in 0-split_size0")
        if not (0 <= self.shift_size[1] < self.split_size[1]):
            raise ValueError("DAT: shift_size must in 0-split_size1")
